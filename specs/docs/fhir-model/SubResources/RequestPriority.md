# Resource Documentation: Request Priority

## 1. Overview
A `RequestPriority`, in the context of `Communication`, indicates the urgency of the correspondence. Whilst for the majority of vaccination letters will be sent as a `routine` vaccination, there may be specific cases in which an `urgent` correspondence is required, such as if the patients immunity is critical to their health. This theoretically could be used to prioritise the order in which letters are sent.

To ensure consistent and accurate recording of the urgency of a `Communication` across different healthcare systems, it is essential that we standardise the reference data for indicating a `RequestPriority`. (E.g. Phone) Leveraging the FHIR value set codes within the FHIR value set will allow our system to align with international healthcare standards, and make it easily interoperable. We can create the `RequestPriority` table that stores this reference data, and allow our consuming applications to reliably store and retrieve this information, enabling users to document this data in a way that is universally recognised. This approach will enhance data quality, support easier interoperability with other systems, and provide a basis for universally understandable reporting and analytics.
...
- The `RequestPriority` Value Set should correlate to the [RequestPriority - HL7](https://build.fhir.org/valueset-request-priority.html).
...
### 3.2. Relationships
- Referenced in the `Communication` resource via the `PriorityId` field on `Communication`
---
## 4. Context and Usage
### 4.1. Scenarios
- Used to indicate the urgency of a `Communication`.
...
- The system field should default to the FHIR system URI and be non-null. ("http://hl7.org/fhir/request-priority")
