# Resource Documentation: Event Status

## 1. Overview
An `EventStatus` is used to indicate the current stage in the lifestyle of an event. For example, in the context of a `Communication`, a letter in draft would be marked as `in-preparation`, when being sent, it may change to `in-progress`, and when the letter is receieved, it will be marked as `Completed`. This is a common workflow in the Welsh Immunisation System for correspondence.

To ensure consistent and accurate recording of the current status of a `Communication` across different healthcare systems, it is essential that we standardise the reference data for indicating a `EventStatus`. (E.g. Active) Leveraging the FHIR value set codes within the FHIR value set will allow our system to align with international healthcare standards, and make it easily interoperable. We can create the `EventStatus` table that stores this reference data, and allow our consuming applications to reliably store and retrieve this information, enabling users to document this data in a way that is universally recognised. This approach will enhance data quality, support easier interoperability with other systems, and provide a basis for universally understandable reporting and analytics.
...
- The `EventStatus` Value Set should correlate to the [EventStatus - HL7](https://build.fhir.org/valueset-event-status.html).
...
### 3.2. Relationships
- Referenced in the `Communication` resource via the `StatusId` field on `Communication`
---
## 4. Context and Usage
### 4.1. Scenarios
- Used to indicate the current status of a Correspondence.
