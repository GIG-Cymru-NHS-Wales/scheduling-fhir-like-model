...
## 1. Overview
A `CommunicationCategory` is utilised as a way of categorising the different types of `Communications` that can exist within a Healthcare communication. For example, in the context of the Welsh Immunisation System, a patient may receive a `notification` to indicate that they have been scheduled for an appointment, with a follow-up `reminder`.

To ensure consistent and accurate recording of the categorisation of a `Communication` across different healthcare systems, it is essential that we standardise the reference data for indicating the `CommunicationCategories`. (E.g. reminder) Leveraging the FHIR value set codes within the FHIR value set will allow our system to align with international healthcare standards, and make it easily interoperable. We can create the `CommunicationCategories` table that stores this reference data, and allow our consuming applications to reliably store and retrieve this information, enabling users to document this data in a way that is universally recognised. This approach will enhance data quality, support easier interoperability with other systems, and provide a basis for universally understandable reporting and analytics.
...
- The `CommunicationCategory` Value Set should correlate to the[CommunicationCategory - HL7](https://build.fhir.org/valueset-communication-category.html).
...
- The system field should default to the FHIR system URI and be non-null.("http://terminology.hl7.org/CodeSystem/communication-category")
